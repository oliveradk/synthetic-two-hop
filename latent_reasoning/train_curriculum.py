...
def run_hf_finetuning(
    ...
    train_dataset_groups = {
        ...
        for dataset_group in custom_args["train_datasets"]
    }
    for dataset_group_name, datasets in train_dataset_groups.items():
        for dataset_name, dataset in datasets.items():
            print(f"Loaded {dataset_group_name=} {dataset_name=} with {len(dataset)=}")

    train_datasets = {
        dataset_group_name: concatenate_datasets(list(datasets.values()))  # type: ignore
        for dataset_group_name, datasets in train_dataset_groups.items()
    }
    for dataset_group_name, train_dataset in train_datasets.items():
        print(f"Concatenated {dataset_group_name=} into one with {len(train_dataset)=}")
    ...
    for i, (dataset_group_name, train_dataset) in enumerate(train_datasets.items()):
        if my_rank == 0:
            run = wandb.init(
